        ...
        <div class="px-5 py-4 hover:bg-white/[0.02] transition-colors">
            <div class="flex items-start justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl {{ $record->record_type === 'consultation' ? 'bg-[#1392EC]/10' : ($record->record_type === 'dental' ? 'bg-blue-500/10' : 'bg-amber-500/10') }} flex items-center justify-center">
                        <span class="material-symbols-outlined {{ $record->record_type === 'consultation' ? 'text-[#1392EC]' : ($record->record_type === 'dental' ? 'text-blue-400' : 'text-amber-400') }}" style="font-size:20px;">
                            {{ $record->record_type === 'dental' ? 'dentistry' : 'clinical_notes' }}
                        </span>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-white capitalize">{{ $record->record_type }} Record</p>
