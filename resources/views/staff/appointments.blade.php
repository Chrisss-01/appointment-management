...
<div class="flex gap-2 mb-6 overflow-x-auto">
    @foreach(['pending' => 'Pending', 'approved' => 'Approved', 'completed' => 'Completed', 'no_show' => 'No Show', 'expired' => 'Expired', 'closed' => 'Closed', 'all' => 'All'] as $key => $label)
...
<div class="bg-[#1A1A1A] border border-white/5 rounded-2xl overflow-hidden">
                ...
                <tr class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-5 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            @elseif(in_array($apt->status, ['cancelled', 'cancelled_by_staff', 'rejected', 'expired']))
                            <button onclick="showReasonModal('{{ $statusLabel }}', '{{ addslashes(match($apt->status) { 'rejected' => $apt->rejection_reason, 'expired' => $apt->expiry_reason, default => $apt->cancellation_reason }) }}')" class="text-xs text-gray-400 hover:text-gray-300 transition-colors">
                                View Reason
                            </button>
                            @else
    ...
    <div class="px-5 py-3 border-t border-white/5">{{ $appointments->links('vendor.pagination.appointments-dark') }}</div>
    @endif
</div>
...
<div id="reject-modal" class="fixed inset-0 bg-black/60 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-[#1A1A1A] border border-white/10 rounded-2xl p-6 w-full max-w-md">
        ...
        <form id="reject-form" method="POST" x-data="{ submitting: false }" @submit="submitting = true">
            @csrf @method('PATCH')
            ...
            <div class="flex justify-end gap-3 mt-4">
                ...
                <button type="submit" 
                    :disabled="submitting"
                    class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-medium rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 min-w-[100px]">
                    <template x-if="submitting">
